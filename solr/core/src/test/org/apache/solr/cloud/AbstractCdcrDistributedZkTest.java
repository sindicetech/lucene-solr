...
//    }
  ...
  protected CloudSolrServer createCloudClient(String defaultCollection) {
    try {
      ...
      log.error("Unable to create cloud client for collection: {}", defaultCollection);
    }
    return null;
  }
  ...
  protected List<String> startServers(int nServer) throws Exception {
    ...
    for (int i = 1; i <= nServer; i++) {
      ...
      File jettyDir = createTempDir("jetty" + i);
      jettyDir.mkdirs();
  ...
  private File createTempDir(String id) {
    Random r = new Random();
    return new File(TEMP_DIR, getClass().getName() + "-" + id + "-" + r.nextLong());
  }
